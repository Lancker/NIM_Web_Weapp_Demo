...
Page({
  ...
  onLoad: function (options) {
    ...
    console.log("===========chatToMessageList tempArr===========");
    console.log(tempArr);
    console.log("===========chatToMessageList tempArr===========");
        ...
        app.globalData.nim.getUser({
          account: userAccount,
            ...
            for(var i=0;i<tempArr.length;i++){
              if (tempArr[i].from == userAccount){
                tempArr[i].chatToLogo = chatToLogo;
              }
            }
    ...
    app.globalData.subscriber.on('RECEIVE_P2P_MESSAGE', ({ account, time }) => {
      ...
      var userAccount = newMessage.from;
      app.globalData.nim.getUser({
        account: userAccount,
        done: function (err, user) {
          if (err) {
  ...
  sendBigEmoji(val) {
    ...
    app.globalData.nim.sendCustomMsg({
      scene: this.data.chatTo.length < 20 ? 'team' : 'p2p',
      to: self.data.chatTo,
